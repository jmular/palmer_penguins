...
First you have to set up your R environment by loading the 'tidyverse' and palmer penguins' and other packages to be used
...
To use a package you must load them first
...
Say you want a quicker way to look at this data where you can determine from a quick glance
...
##Question 2: What are the different relational features involving sex?
...
H1L (alternative hypothesis) two variables are not independent (i.e. "There is an effect in the population")

```{r chisq.test for species v. sex}
chisq.test(table(penguins$sex, penguins$species)) 
```
Since the p-value (0.0008047) of the text is GREATER than 0.05, is not statistically significant and indicates strong evidence for the null hypothesis
...
##geom_smooth
geom_smooth leads to line plots; helpful for showing general trends in data

There is two kinds (methods) of smoothing
  ...
  R will try and compute whichever method it deems appropriate but it's good to get in the habit of noting things you need to.
...
geom_point and geom_smooth can also be combined in a singular visualization
...
##Cleaning Up Code
  If you notice your code getting a bit complicated (especially with two ggplots) you can change your code of the aes (aestetics) to the ggplot area like so:
...
```
###Grouping Data
To group a set of data from your code (for example type of species) we can add that information within our aes area. We can use color, size, and shape to distinguish
...
count_species <- function(x, value) { #first turn into a vector for easy usage
...
```{r}
count_species(penguins$species, "Adelie")
